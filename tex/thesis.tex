...
In addition to extracting ratings, a similar approach was used with \emph{gemini-2.5-flash-lite} to extract quantitative cost-effectiveness-related outcomes. %claudesuggests: Discrepancy 1 (cost-effectiveness audit): code (C_extract_quantitative_information.py line 116) uses MODEL_NAME = "gemini-2.5-flash-lite" for the initial extraction step; gemini-2.5-flash is used only in the subsequent aggregation step (E_gemini_aggregate_quantitative_outcomes.py). 
...
Using manual inspection and frequency statistics of common bigrams, a set of outcome categories was defined that recur across evaluations and are interpretable across projects. The forecasting pipeline includes all outcome categories for which sufficient data could be extracted; category inclusion is hardcoded based on manual inspection rather than enforced by a programmatic count threshold. The set used in the forecasting model included: %claudesuggests: Discrepancy 3 (cost-effectiveness audit, HIGH): C_predict_outcomes_saves_model.py ONLY_USE_THESE_AND_ASSERT_EXIST_WITH_COUNTS includes 16 categories, including all categories previously listed as "excluded" (yield, forest/area indicators, pollution load, stoves, trees). The minimum count threshold in code is MIN_TRAIN_ROWS_PER_GROUP=1 and MIN_TEST_ROWS_PER_GROUP=1 (not 10). The >=10 claim is not programmatically enforced.
\begin{itemize}[noitemsep,topsep=0pt]
  ...
  \item \textbf{Forest and area indicators:} trees/seedlings planted (counts) and area-based outcomes (reforested, under management, protected) in hectares.
...
All extracted outcome groups per category per activity were sent to an aggregation prompt for \emph{gemini-2.5-flash} regardless of whether multiple values were present (single-record pass-through was disabled in the pipeline). The model was instructed to produce a single coherent project-level outcome, distinguishing duplicates from additive components, prioritizing project-level totals over subcomponent figures, aggregating only non-overlapping quantities, and returning a null response when no coherent representative value could be determined. %claudesuggests: Discrepancy 2 (cost-effectiveness audit): code (E_gemini_aggregate_quantitative_outcomes.py lines 196-218) has the single-record passthrough branch commented out; Gemini is called for every group including singletons, not only multi-value groups as previously implied.

\textbf{Outcome aggregation and cost-effectiveness computation}

Once outcomes were extracted and aggregated to project level, cost-per-unit estimates were constructed. The total expenditure for each activity (column \texttt{actual\_total\_expenditure} from IATI) served as the cost denominator (in USD), with the exception of benefit-cost ratios and rates of return, which are inherently unit-free or already normalised. Agricultural yield outcomes do receive a proportional funding allocation, but the resulting dollar-per-unit column is not used for cost-effectiveness aggregation. %claudesuggests: Discrepancy 14 (cost-eff audit): denominator column is actual_total_expenditure, not a disbursement-specific field (IATI distinguishes disbursements from expenditure). Discrepancy 5: yield is NOT in NO_MULTIPLIER; it gets a funding multiplier but the DPU column is excluded downstream via ~dpu_is_yield.
...
Carbon dioxide reductions are handled as a special case because they can act as a summary metric that overlaps with other mitigation outputs. If CO$_2$ reductions are reported without any closely linked mitigation outputs (such as improved stoves, added generation capacity, trees planted, reforested area, or area under management), CO$_2$ reductions receive an equal share like any other bucket. If CO$_2$ reductions are reported alongside any of those linked outputs, they inherit the combined allocation already assigned to the CO$_2$-mitigating expenditures for that activity, preventing CO$_2$ from inflating allocated spending when it is a co-reported consequence of other outcomes. %claudesuggests: Discrepancy 6 (cost-eff audit): code (split_up_activity_funding.py line 14) CO2_SUBS = {"stoves","generation_capacity","trees_planted","area_reforested","area_under_management"}; area_reforested and area_under_management were omitted from the original description.

Empirical inspection found that most cost-per-unit outcomes are approximately log-normally distributed. The $\log_{10}$ transform was therefore applied to all outcome categories except benefit-cost ratios, rates of return, and agricultural yield outcomes before modelling. %claudesuggests: Discrepancy 4 (cost-eff audit): code (C_predict_outcomes_saves_model.py lines 1003-1033) also excludes yield from the log10 transform alongside B/C and RoR; the is_yield flag uses linear scale. E_process_quantitative_results_with_keywords.py line 550 confirms yield is excluded from log_plot.
...
Of the 23 tags, 12 were interpreted as not having occurred if not mentioned. These are applicable to any project and would be very likely to be mentioned in the summary if it occurred. For example, whether the project was delayed. The remaining 11 tags were not recorded as false if missing. Instead, information as to whether they occur was recorded only if explicitly mentioned, such as having successfully completing or failing to complete infrastructure, or whether livelihoods were significantly achieved by the project. For unsigned tags, the LLM is asked whether the phenomenon was clearly described as occurring, and the response is a single boolean per tag. %claudesuggests: DISC-T-01 (ratings/tags audit): code (final_tags.json) has 12 entries with signed=false (unsigned) and 11 with signed=true; previous text had the counts swapped.
...
\begin{itemize}[noitemsep,topsep=0pt]
 ...
 \item the World Governance Indicators scores \citep{kaufmannWorldwideGovernanceIndicators2024} for control of corruption, government effectiveness, political stability, regulatory quality, and rule of law (5 indicators), plus a composite mean of these five scores. When the composite is present the five individual WGI columns are removed from the feature set (replaced, not supplemented) (consultation with BMZ officers), \citep{goldembergMindingGapAid2025} \citep{ndikumanaImpactForeignAid2017} %claudesuggests: DISC-R-07: code (C_run_GLM_rating.py lines 1503-1504) drops individual WGI cols when governance_composite is added; the original phrasing "as well as the mean" implied both are retained simultaneously, which is incorrect for the ratings model.
...
Similarity was first weighted proportional to its embedding semantic similarity score, and a cutoff was tested for averaging 1, 7, and 20 nearest neighbors using the Gemini embeddings model \emph{gemini-embedding-001}, embedded on the activity title and LLM-generated summary (not on the targets text). 20 nearest neighbors was found to be the highest-performing number of neighbors using this method. %claudesuggests: D7 (narrative audit): code (get_similar_activities.py) uses activity_text_embeddings_gemini.jsonl which embeds title+summary; the targets embeddings file is commented out. D8: FEW_SHOT_KS = [1, 7, 20] in code; values 3, 10, 15 were not tested and 15 was not the optimum. The weighted average of the nearest neighbor ratings is used to predict the overall activity score.
...
\begin{itemize}
 \item \emph{gpt-3.5-turbo}-generated evaluation on a score from 0 to 100 of: %claudesuggests: DISC-R-08: code (A_grade_baseline_features_gpt3p5.py line 463) calls model="chatgpt" (OpenAI gpt-3.5-turbo) for all 7 grade prompts; the gemini-2.5-flash label was incorrect.
 \begin{itemize}[noitemsep,topsep=0pt]
...
\begin{itemize}
\item expenditure times complexity %claudesuggests: DISC-R-06 (HIGH): governance_x_complexity does NOT appear in C_run_GLM_rating.py; only expenditure_x_complexity is added for the ratings model. The governance-times-complexity interaction is present in the outcome tags initial feature set but is subsequently dropped as one of the 18 noisy features.
\end{itemize}
\end{itemize}
...
The model for the outcome tag was judged as ineligible for test-set prediction unless the Brier skill score was strictly positive \emph{and} the accuracy skill exceeded 0.5 percentage points above the majority-class baseline (i.e., val\_acc $-$ majority\_acc $> 0.005$). Otherwise the chosen model was reverted to the constant baseline, indicating low confidence that the RF+ET ensemble would perform better than the validation set. %claudesuggests: DISC-T-04/Discrepancy 8 (tags audit, HIGH): code (D_train_staged.py) uses ACC_SKILL_MIN_IMPROVEMENT = 0.005; threshold is strictly > 0.005 (not simply > 0 as previously stated). The constant ACC_SKILL_MIN_IMPROVEMENT = 0.005 is explicitly defined on line 167.
...
An ablation experiment for all tags was conducted on the validation set. The RF+ET model uses approximately 62 independent features (47 explicit base features plus 15 sector cluster columns), which may not be appropriate for all outcome tags. %claudesuggests: DISC-T-08 (ratings/tags audit): code (D_train_staged.py get_feature_cols lines 795-855) lists 47 explicit base features + dynamic sector clusters; with 15 clusters the count is ~62, not 63. A series of experiments were conducted to assess which features should be used per-tag.
...
The correction multiplier was validated by sweeping values in $\{0, 0.1, 0.2, 0.5, 1.0, 1.5, 2.0\}$ and evaluating mean within-group pairwise ranking, Brier skill, and accuracy skill on the validation set across all corrected tags. 
...
\begin{itemize}
    \item Success Group: average of six outcome tags (targets met,
          beneficiary satisfaction, private sector engagement, capacity building, policy reforms,
          and financial performance). Note: Improved Financial Performance is an \emph{unsigned} tag (not a signed tag) despite being included in this success-oriented group. %claudesuggests: Discrepancy 9 (tags audit): final_tags.json has improved_financial_performance with signed=false; the group description of "signed tags" was inaccurate.
    \item Financing Group: disbursement tag, funds-cancelled tag (inverted).
    \item Rescoping Group: Project Restructured, Targets Revised, Closing Date Extended, Funds Reallocated. This group's RF+ET regressor is trained and its predicted score is used as an OLS predictor input when reconstructing per-tag probabilities for blended tags, but rescoping-group tags are not directly blended (the group influences other tags via OLS rather than being a direct mixture target). %claudesuggests: DISC-T-06/Discrepancy 10 (tags audit): code (D_train_staged.py MANUAL_FACTORS lines 327-352) defines factor_rescoping; it is trained and its predictions enter the OLS reconstruction, affecting other tag probabilities. The spec only mentioned two groups; the third was entirely absent.
\end{itemize}
...
An LLM-based correction step was tested for tag forecasting, analogous to the approach described for the overall rating model. For each of the 14 tags whose statistical model beat the constant baseline, \emph{deepseek-reasoner} was prompted with the activity title, an LLM-generated summary of the pre-activity documents, the tag label and definition, and the training-set base rate. %claudesuggests: DISC-T-05/Discrepancy 7 (tags audit): code (K_llm_tag_forecasts.py line 65) MODEL = "deepseek-reasoner"; this is DeepSeek's chain-of-thought reasoning model, distinct from deepseek-V3.2 (the chat/completion model used for narrative forecasts). The model was instructed to reason like a superforecaster: enumerating reasons the outcome was likely true, then likely false, then anchoring on the base rate before producing a final probability. Forecasts were produced for 150 recent training activities and all validation activities. The LLM probabilities were then blended with the statistical model predictions (80\% statistical, 20\% LLM). A sweep of blend weights from 0\% to 70\% LLM was conducted, measuring mean within-group ranking skill, accuracy, and Brier skill score across the 14 non-baseline tags on the validation set.
...
as the forecasting model for overall ratings, due to its performance on benchmarks and low cost. \emph{gemini-2.5-pro} was also selected due to its performance. Lastly \emph{gemini-2.5-flash
...
The LLM forecasting method was decided upon by iteratively inspecting both the quality of the response, and the overall accuracy of the forecasts made by the LLM. To generate the LLM forecasting methods, \emph{gemini-2.5-flash} was prompted with a series of ``mock forecasts'', generated by \emph{gemini-2.5-flash}. %claudesuggests: D11 (narrative audit, HIGH): no call to gemini-2.5-pro found in mock forecast pipeline (C_generate_reasons_for_ratings.py); default model in extracting_and_grading_helper_functions.py line 100 is gemini-2.5-flash. The ``mock forecast'' used relevant pages retrieved by ranking the categorized topics by forecast informativeness and retrieving 10 pages of the most relevant activity data and up to 5 pages of the most relevant evaluation data, prioritizing pages marked as ``deviations from plans'', ``delays or early completion'', or ``over or under spending'' with a minimum forecasting relevance score of 3/10, and otherwise returning the pages with the highest forecasting relevance score. %claudesuggests: D13 (narrative audit): code (B_fine_tune_2p5_pro.py) uses top_k_per_activity=5 for outcome/evaluation pages, not 10 as previously stated.

To generate each mock forecast, a retrieval-augmented input was constructed consisting of up to 10 baseline pages and up to 5 outcome/evaluation pages per activity. Baseline pages were selected from high-scoring passages in predefined ``forecast-informative'' categories (e.g., objectives, implementation plans, risks, financing details, contextual challenges, and stakeholder/implementer information), using a relevance threshold (minimum categorization score of 3) and including nearby pages when insufficient high-scoring pages were available. %claudesuggests: D12 (narrative audit, HIGH): all code uses min_score=3 throughout (L_summarize_expost.py, B_fine_tune_2p5_pro.py); no instance of min_score=9 found anywhere; the earlier "minimum categorization score of 9" was incorrect. Pages were selected from post-activity documents which emphasized deviations from plans (including deviations, delays/early completion, and over/under-spending), using a lower relevance threshold (minimum score of 3) and likewise included surrounding pages to reach the target count when needed. These retrieved excerpts were then merged with activity metadata (title, scope, planned start/end dates, planned financing totals when present) and brief model-generated baseline summaries (activity description and risk summary) before prompting Gemini to write a forecast from the ex-ante perspective. Importantly, the prompt required the model to end by outputting the \emph{known} final evaluation rating for that activity (derived from the merged ratings file and converted into scale-specific text via \texttt{get\_ratings\_text}), while also instructing it to ground the narrative in the retrieved evaluation pages and to return ``NO RESPONSE'' if the evaluation excerpts did not contain sufficient justification for the assigned rating.
...
\begin{enumerate}[noitemsep,topsep=2pt]
  \item \textbf{Activity metadata} --- title, planned start/end dates, scope, planned disbursement (USD), location(s), GDP per capita, reporting organisation. %claudesuggests: D4 (narrative audit): code (B_forecast_with_few_shot.py) uses meta.get("reporting_orgs") in the KNN prompt builder; "participating and implementing organisations" is used in the baseline prompt path but not the KNN path. The KNN-based prompt (which is the main described method) uses reporting_orgs.
  ...
  \item \textbf{Multi-stage reasoning (S1/S2)} --- outputs of two prior LLM calls: reasons the activity may have been rated ``Moderately Satisfactory'' or worse (S1) and reasons it may have been rated ``Satisfactory'' or ``Highly Satisfactory'' (S2). %claudesuggests: D5 (narrative audit, HIGH): code (B_forecast_with_few_shot.py S2 stage) asks for reasons why "Satisfactory" or "Highly Satisfactory" --- skipping Moderately Satisfactory entirely; S2 is NOT "MS or better" but rather "S or HS only", making it asymmetric with S1.
...
\end{enumerate}
%claudesuggests: D6 (narrative audit): code (B_forecast_with_few_shot.py) injects rating_text_dist into the scratchpad_method instruction, not as a terminal block 7; S1/S2 outputs are injected inline in the s3 stage; the sequential "7-block structure with rating dist last" description does not match the actual code assembly order.
...
In both methods, a $k$-nearest-neighbors (KNN) few-shot block of semantically similar activities in the training data is used (see Section \ref{ssub:nearest_neighbor_vector_similarity} for how semantic similarity was determined). A range of nearest neighbors was evaluated. The language model was prompted to extrapolate lessons about rating scales for the most similar ``Highly Unsatisfactory'', ``Unsatisfactory'' or ``Moderately Unsatisfactory'', the most similar ``Moderately Satisfactory'', and the most similar ``Satisfactory'' or ``Highly Satisfactory'' rated examples in the training data. A selection of $k=3$ summarized mock forecasts was found to perform better than $k=1$ or $k=7$ (tested values were $k \in \{1, 7, 20\}$). %claudesuggests: D9 (narrative audit): code (B_run_many_different_forecast_with_fewshot.py, B_generate_rag_forecasts.py) shows FEW_SHOT_KS in the sweep; values 5 and 7 were not both tested; the tested set is [1, 7, 20], not [1, 5, 7].

Each example activity in the few-shot block included (i) the activity title, (ii) a short ``risks'' summary, (iii) the retrospective mock forecast analysis, and (iv) the final evaluation outcome label. %claudesuggests: D10 (narrative audit): code (build_few_shot_block in B_forecast_with_few_shot.py) appends only title, risk summary, mock forecast text, rating scale options, and outcome label; location and brief summary variables are loaded but NOT added to the few-shot block text.

\textbf{Additional Prompts}
Two additional prompts were given, and inserted into the final forecast: (1) reasons the activity may have been evaluated as ``Moderately Satisfactory'' or worse (S1), (2) reasons the activity may have been evaluated as ``Satisfactory'' or ``Highly Satisfactory'' (S2).

The forecasting prompt required a structured 6-step response (scratchpad method): (1) provide reasons the overall success might be ``Moderately Satisfactory'' or worse, (2) provide reasons it might be ``Satisfactory'' or ``Highly Satisfactory'', (3) produce an initial forecast from those considerations, (4) check whether the initial forecast is an extreme outlier, (5) reconsider and finalize the forecast in light of that check, and (6) output the final forecast on the last line beginning with \texttt{FORECAST: }. %claudesuggests: D14 (narrative audit): code (B_forecast_with_few_shot.py scratchpad method) defines 6 numbered steps including an extremity-check step (4) and a reconsideration step (5); the original description of a "4-step format" omitted these important reasoning steps.
...
In past work in a similar domain, fine-tuning significantly improved forecasting performance \citep{wenPredictingEmpiricalAI2025}. An attempt was made to fine-tune \emph{gemini-2.5-flash} using Vertex AI's preference tuning API (analogous to Direct Preference Optimization, DPO), which requires one example of a good prompt, and one example of a bad prompt. %claudesuggests: D18 (narrative audit): code (B_fine_tune_2p5_pro.py line 950) uses "method": "PREFERENCE_TUNING" (Vertex AI API term); the code does not use the term "DPO" explicitly. Preference tuning is related to but not identical to DPO. Out of a random sample of 100 activity IDs in the training data, the final forecast stage was forecasted 5 separate times using \emph{deepseek-V3.2}. 50 pairs were used where the model forecasted one rating increment closer than another rating, as the pairs of forecasts.
...
\]
where $\lambda$ is a scaling factor (set to 1.0 in the experiments) and clipping is applied to the LLM residual corrector output at $[0, 5]$ (the mode-demeaned residual space). %claudesuggests: DISC-R-03 (ratings audit): final predictions from the RF+ET model (before LLM correction) are clipped to [1.0, 6.0] to match the actual 1--6 rating scale; the LLM corrector defaults clip to [0.0, 5.0], which could cut off valid predictions in the 5--6 range for activities above the mode. The clip [0,5] here refers to the residual space, not the raw rating space. DISC-R-02: the year correction for ratings (linregress / OLS) does not apply a clip after adding the correction; only the LLM corrector applies clipping.

The ridge model takes both the RF+ET prediction and the LLM Forecast as inputs for activities where $\hat{y}^{\text{LF}}_i$ is available. The effective fit set is all training activities for which both RF and LLM predictions are non-null --- approximately 300 activities when LLM coverage is at roughly that level --- but no hard cap of exactly 300 is enforced programmatically; the count is an emergent property of LLM coverage. %claudesuggests: DISC-R-10 (ratings/tags audit): code (C_run_GLM_rating.py line 1866) passes the full train_idx as meta_train_idx; the ~300 figure reflects LLM coverage sparsity, not an explicit limit. DISC-R-11: the spec internally inconsistently uses 268 (§2 table) and 300 (§7) for validation set size; the 300 in the LLM corrector context refers to training+recent activities, not the pure validation split. This allows the correction to learn a mapping from $(\hat{y}^{\text{RF}}_i, \hat{y}^{\text{LF}}_i)$ to the residual $r_i$. The ridge model under this setting effectively learns how much LLM forecasts should shift the prediction from the mean, how much the RF+ET model should shift the prediction from the mean, as well as a fixed learned offset. The only learned parameters are $\beta_0$, $\beta_1$ and $\beta_2$.

\subsection{Scoring Metrics} \label{sub:scoringmetrics}

\textbf{Accuracy}
The percent of the time the correct rating is forecasted. Non-integers are rounded to integers for overall ratings. While accuracy has the advantage of intuitive simplicity, is not strictly proper \citep{gneitingStrictlyProperScoring2007}, meaning that maximizing the model accuracy may not maximize a model's reported true belief about the probability of an outcome. This can be problematic when being used to select between models.
...
Measure of “average” distance, by taking the mean of the absolute value of the residuals. Lower is better. On a scale from 0 to 5, therefore worst possible value is 5, best possible value is zero. This method does not heavily penalize predictions that are significantly incorrect. MAE is only used in this work for overall ratings.
...
Four methods for measuring activity outcomes were compared.
...
\textbf{Cost-effectiveness} Activity ratings show only weak correlation with activity aggregate cost-effectiveness (Pearson $r = \RatingCostCorr$, computed across the full dataset not the held-out set alone). %claudesuggests: Discrepancy 9 (cost-eff audit, HIGH): code (C_predict_outcomes_saves_model.py lines 4822-4841) comment explicitly states "zagg_true for ALL activities (not just test)"; the correlation is full-dataset, not held-out-set. Cost-effectiveness prediction was not shown to be statistically significant. The cost-effectiveness aggregate Z-score is standardised using training-set mean and standard deviation (population stddev, ddof=0) applied to all splits, and the Random Forest used for cost-effectiveness forecasting is trained with 3$\times$ sample weight on the most recent 20\% of training observations. %claudesuggests: Discrepancy 7 (cost-eff audit): Z-score uses train-only mu/sd to prevent leakage; this is not the same as global standardization. Discrepancy 11: recency weighting (sample_weights[cutoff_idx:]=3.0) is undocumented. The cost-effectiveness aggregate Z-score is limited by inconsistency in its component indicators across activities, difficulty of interpretation, and low sample counts, indicating a need for a larger training set or a less noisy data collection method for international aid cost-effectiveness forecasting. 
...
When dividing by each activity's total expenditure (IATI field \texttt{actual\_total\_expenditure}) for those that are marked as dollar-per-unit, and looking at the aggregate cost-effectiveness outcomes, the correlation on Z-scores is not statistically significant above the baseline. Cost-effectiveness is forecast using a Random Forest regression on Z-scores (not the same logit/ordinal model used for ratings). %claudesuggests: Discrepancy 8 (cost-eff audit, HIGH): code (C_predict_outcomes_saves_model.py lines 4645-4652) uses run_random_forest_median_impute_noclip for zagg; ratings use logit+ordinal models. The claim "same models as ratings" in the spec is directly contradicted by the code. Discrepancy 14: denominator is actual_total_expenditure, not a disbursement-specific column.
...
For each tag assigned a reduced feature subset, k-fold performance was at least as good as the full-feature alternative, indicating that the reduction reflects a real generalisation benefit rather than capitalising on the specific characteristics of the validation period. However, given the aggressive information reduction available to the model, the test set was also evaluated without per-tag feature selection.
...
The base configuration (\texttt{min\_samples\_leaf}=5, \texttt{max\_features}=\texttt{"sqrt"}, \texttt{n\_estimators}=500, \texttt{max\_depth}=None, \texttt{min\_samples\_split}=2, \texttt{ccp\_alpha}=0), where bootstrap is true for the RF and false for the ET, was found to be the best performing default. Ten individual tags have per-tag hyperparameter overrides: three tags use \texttt{max\_depth}=10 (rather than None) and several use \texttt{min\_samples\_leaf} values of 10, 20, or 40 depending on the tag's data characteristics. %claudesuggests: Discrepancy 5 (tags audit): code (D_train_staged.py TAG_RF_PARAMS_OVERRIDES) defines overrides for 10 tags including tag_high_disbursement (leaf=20), tag_over_budget_success (leaf=40), tag_high_beneficiary_satisfaction_or_reach_success (depth=10), among others; the base parameter table is not universal. Although quite similar to the default \texttt{RandomForestClassifier} and \texttt{ExtraTreesClassifier} configuration, this specification still constrains model capacity in ways that reduce overfitting on the small training sets involved: requiring at least five training examples at each leaf limits fine-grained partitioning of the feature space, and restricting the feature subset considered at each split (\texttt{max\_features}=\texttt{"sqrt"} rather than considering all features) increases tree diversity. Compared to defaults, these choices generally accept less precise fits on the training data in exchange for predictions that are more stable across unseen examples.

\textbf{Start-year trend correction}

The start year correction was applied to all tags except \texttt{tag\_monitoring\_and\_evaluation\_challenges} and \texttt{tag\_high\_disbursement}, where a year trend hurt performance on the validation set. %claudesuggests: DISC-T-03/Discrepancy 6 (tags audit): code (D_train_staged.py lines 52-55) SKIP_START_YEAR_CORRECTION_TAGS = {"tag_monitoring_and_evaluation_challenges", "tag_high_disbursement"}; both tags skip the correction but only one was previously mentioned. Confirmed by the results table where High Disbursement shows Tmp. Corr.: None.

The correction multiplier of $1.0$ (full correction to the RF component) achieved the best pairwise ranking, Brier skill, and accuracy, and was therefore retained. However, it was found that applying the correction to the RF model alone on the validation set and only then averaging the result with the ET performed better than correcting the average, so the temporal correction was only applied to the RF model before averaging with ET. Because only the RF component is corrected and the ET remains uncorrected, the net effect on the final RF+ET ensemble prediction is half the fitted correction magnitude (i.e., the ensemble sees correction $\div$ 2). The multiplier sweep over $\{0, 0.1, 0.2, 0.5, 1.0, 1.5, 2.0\}$ therefore corresponds to ensemble-level correction magnitudes of $\{0, 0.05, 0.1, 0.25, 0.5, 0.75, 1.0\}$. %claudesuggests: DISC-T-11/Discrepancy 11 (tags audit): code (D_train_staged.py lines 50-51) comment explicitly states "so the net effect on the ensemble is correction/2"; the spec/thesis did not mention this halving consequence, which is material for understanding calibration.
