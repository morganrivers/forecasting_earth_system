...
All activities for the thesis come from the IATI database \citep{IATIDashboardIATI}. Country-level data were enriched with the World Bank governance indicators \citep{kaufmannWorldwideGovernanceIndicators2024}, the World Bank country policy institutional assessment scores \citep{worldbankCPIACriteria2024}, and GDP per capita \citep{worldbankGDPPerCapita}.

Several other sources were also evaluated as alternatives but ultimately not used:
\begin{itemize}
\item The OpenAlex publication repository of peer-reviewed evaluation documents and abstracts \citep{priemOpenAlexFullyopenIndex2022}
\item The AidData database of international aid activities \citep{tierneyMoreDollarsSense2011}
\item The 3ie development database of ex-post evaluations \citep{3ieDevelopmentEvidence}
\item The IDEAL database of ex-post evaluations \citep{kfwdevelopmentbankIDEaL}
\end{itemize}

The IDEAL database and 3ie each had fewer than 300 records for environmental topics each, far lower than IATI. Furthermore, although many millions of evaluations are available on OpenAlex, it proved difficult to reliably identify and de-duplicate published evaluations of environmental interventions. 

The IATI database was used due to its substantial quantity of information available in textual format and extractable from the database records, its large quantity of activities, and the fact that new records are continuously added, making rerunning experimental methods in the future possible with little modification.
...
Finally, the total disbursement for the activity reported by IATI was used to determine an approximate estimate of the cost in US Dollars per unit outcome (see ``Splitting Disbursements'' below), with the exception of benefit-cost ratios, rates of return, and agricultural yield outcomes. Empirical inspection found that most outcomes were approximately log-normally distributed. The $log_{10}$ of all categories except Benefit-Cost ratios and rates of return was then applied.


\textbf{Splitting disbursements}

Unfortunately, outcome-level funding splits from the IATI database were not available. In order to represent the fact that dollar-per-unit spending can be allocated across several outcomes, a custom algorithm was written to separately allocate total activity expenditures to non-overlapping funded sub-activities. The procedure assigns each activity's total expenditure across the outcome components it reports, so later cost-per-unit calculations do not implicitly treat multi-outcome activities as having multiple full budgets. Benefit/cost ratios and economic and financial rate of return are excluded from monetary allocation, and other outcomes are eligible for splitting. To avoid double-counting when two indicators are simply alternative measurements of the same underlying result, closely related indicators are first grouped into shared conceptual buckets, such as pairing protected area with area under management, pairing different yield-increase measures, and pairing tree planting with reforested area.
...
\subsubsubsection{LLM Prompting Strategies}
...
Let $\hat{y}^{\text{RF}}_i$ be the RF+ET prediction for activity $i$, and let $\hat{y}^{\text{LF}}_i$ denote the LLM Forecast. Note that the RF+ET prediction was also averaged with the ET model, but it is still referred to as ``RF'' in the equations. The random-forest residual on the i'th activity is defined as:
\[
...
\FloatBarrier
\section{Results}
The sections below discuss the activity overall rating and similarity grade forecasts, and aggregate Z-score forecast results. First, key findings with regards to the best method for evaluating international aid forecasting methods are discussed.

Three key methods for measuring activity outcomes are highlighted: evaluating forecast accuracy of overall evaluation ratings, evaluating forecast accuracy of overall aggregate cost-effectiveness Z-score, and scoring the narrative similarity between LLM forecast and the final outcome. Experimentation revealed several reasons to be wary of activity ratings: they are often gamed as they are influential in future funding \citep{goldembergMindingGapAid2025}, they were found not to correlate strongly with activity outcomes (Pearson score of only 0.07), and models with high forecasting accuracy were found to sometimes have low similarity to the final forecast (see Figure \ref{fig:scatterLLMcorrected}). The cost-effectiveness aggregate Z-score suffers from inconsistency in its component parts between activities, its difficulty of interpretation, and the relatively low numbers of activity cost-effectiveness outcomes. The method of grading narrative similarity was found to be often a superior metric than the other two. It is concluded that evaluating whether narrative forecasts are similar to the post-activity outcome document summary using \emph{gemini-2.5-flash-lite} is the best overall evaluation method. It is low-cost, costing only a few cents for 300 grades, and only takes around 2 minutes for those 300 grades. Furthermore, prior literature has found that the semantic embedding of activity results are correlated with quantitative country-level outcomes \citep{goldembergMindingGapAid2025}, indicating that these are less likely to be ``gamed''.
...
\subsection{Forecasting Ratings and Narrative Forecasts with LLMs}
...
\midrule
RF + LLM Forecast + recency & \textbf{0.255} & \textbf{0.817} & 0.593 & \textbf{0.757} & \textbf{0.547} & 0.724 \\[3pt]
RF + recency & 0.240 & 0.825 & 0.603 & 0.743 & 0.537 & 0.724 \\[3pt]
RF + LLM Forecast + recency (rounded) & 0.219 & 0.836 & \textbf{0.525} & \textbf{0.757} & \textbf{0.547} & \textbf{0.758} \\[3pt]
Random Forest (default params) & 0.215 & 0.839 & 0.628 & 0.723 & 0.530 & 0.715 \\[3pt]
XGBoost, no LLM features & 0.196 & 0.848 & 0.645 & 0.693 & 0.513 & 0.701 \\[3pt]
Random Forest only & 0.192 & 0.851 & 0.645 & 0.693 & 0.507 & 0.724 \\[3pt]
Ridge GLM + Random Forest (mean) & 0.185 & 0.854 & 0.649 & 0.690 & 0.523 & 0.707 \\[3pt]
XGBoost only & 0.184 & 0.855 & 0.638 & 0.693 & 0.523 & 0.702 \\[3pt]
Random Forest, no LLM features & 0.171 & 0.862 & 0.657 & 0.713 & 0.520 & 0.709 \\[3pt]
