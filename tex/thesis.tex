...
\newcommand{\PairwiseHuman}{67}
\newcommand{\PairwiseModel}{72}
\newcommand{\PairwiseCILow}{70}
\newcommand{\PairwiseCIHigh}{76}
...
\textbf{Note: for this draft, the methods and results are the focus, mostly leaving out the introduction, discussion, and conclusion. Feedback is also pending before running the full analysis on the held-out test set.}
...
\begin{enumerate}
  \item  How do judgemental forecasting methods compare to statistical models in forecasting international aid overall success ratings and quantitative outcomes?
  ...
  \item  What aspects of the activity available in the dataset at the beginning of the activity lead to higher or lower ratings?
  \item  How does forecasting aggregate cost-effectiveness compare to forecasting ratings?
\end{enumerate}

To answer these questions, activity outcomes are assessed using three complementary approaches. First, the overall evaluation rating, a numerical score assigned by the funding body at the time of evaluation, serves as the primary quantitative outcome. Second, narrative forecast similarity grades evaluate how closely an LLM-generated qualitative prediction matches the post-activity evaluation summary, providing a measure of forecast quality that is harder to game than ratings alone. Third, aggregate cost-effectiveness Z-scores, derived from quantitative results extracted from evaluation documents, offer a proxy for the relative efficiency of resource use across activities. These three outcome measures structure the empirical comparisons presented in the Results section.

This method differs in two key ways from prior literature, which has analyzed international aid evaluations. The first difference is that while many works in the literature have attempted to assess correlations between quantitative features and aid evaluation ratings, they have not focused on what knowledge would be available at approval for the activity, and do not assess out-of-time generalization of these correlations. Out-of-time generalization refers to model performance on activities that started after those used for training, as opposed to out-of-distribution generalization, which refers to performance on activities from different domains or data sources. This work assesses the out-of-time generalization ability of models. This is critical, because in order to improve aid decision making, one must assess the ability of models to forecast the outcomes of future interventions, not simply statically analyze a corpus of past activities. The second difference is that judgemental AI forecasting is implemented, as a supplement to standard statistical models.

\subsection{Prior Work}

Within the domain of LLM use, there have been early attempts at using them to improve decision making for the environment. A recent tool called ``climsight'' summarizes and aggregates information about climate adaptation and mitigation \citep{koldunovLocalClimateServices2024}, but stops short of making forecasts towards adaptation. Machine learning and LLMs have been used to collect over 80,000 articles about climate adaptation and provide analysis about which areas of implementation are lacking and point out gaps in attention towards promising categories of policies \citep{callaghanMachineLearningMap2025a}.
...
\textbf{Note: A longer section has been summarized here; contact the author if the original is needed.}
...
\FloatBarrier
\subsubsection{Outcome Cost-Effectiveness Extraction}
...
\subsubsection{Outcome Aggregation and Cost-effectiveness Computation}
...
\subsubsection{Outcome Model Training and Evaluation}

For each outcome distribution in Table~\ref{tab:outcome_model_performance}, a random-forest regression model was trained using features and hyperparameters analogous to those used for rating forecasting, with the rating target replaced by the relevant activity-level outcome. A one-hot encoded dummy variable for the outcome category being predicted was also included. Models were trained on activities in the training split with non-missing outcomes and evaluated on the validation split. The counts in Table~\ref{tab:outcome_model_performance} correspond to the number of validation activities available for each outcome. Outcome categories with fewer than 10 activities in either split were excluded.

In addition, a single aggregate Z-score was computed by subtracting the training-set mean and dividing by the training-set standard deviation for each outcome. For each activity, the mean Z-score across all available outcome categories was used as the aggregate cost-effectiveness signal.
...
as the forecasting model, due to its performance on benchmarks and low cost. \emph{gemini-3-pro} was also selected due to its performance. Lastly \emph{gemini-2.5-flash
...
Ensembling averages many individual forecasts of the same model, prompted in slightly different ways. Ensembling was found to be a relatively weak but robust method during validation, so due to its high API cost, the ensembling method is reserved for the final held-out set, to be presented once this thesis has reached completion. 
...
The goal was to correct the RF model for temporal distribution shift (e.g., changing reporting practices, evaluation standards, portfolio composition, and macro conditions). A second goal was to incorporate any usable information from the direct LLM forecasts. Even when the input features are stable, the relationship between those features and the outcome can drift over time, so a model trained on older activities can become mis-calibrated on newer ones. Furthermore, the LLM forecasts were found to be significantly correlated with prediction error in the validation set.
...
\]
with an $\ell_2$ penalty on $(\beta_1,\beta_2)$ controlled by $\alpha$, the ridge strength. The corrected prediction is:
\[
...
This is a simple stacked model: the RF+ET provides the base signal, and ridge regression learns an adjustment to remove systematic residual error that appears in the recent training data.

Two separate methods were tested:
\begin{enumerate}
\item Recency correction (RF+ET re-calibration on recent activities).
In this variant, the LLM forecast is removed entirely, fixing $\beta_2=0$, but an offset $\beta_0$ and scaling $\beta_1$ are still calculated on the 150 latest training examples.

\item LLM-informed correction (recency + LLM Forecast).
In this variant, the ridge model takes both the RF+ET prediction and the LLM Forecast as inputs for activities where $\hat{y}^{\text{LF}}_i$ is available (the 150 latest starting activities). This allows the correction to learn a mapping from $(\hat{y}^{\text{RF}}_i, \hat{y}^{\text{LF}}_i)$ to the residual $r_i$. The ridge model under this setting effectively learns how much LLM forecasts should shift the prediction from the mean, how much the RF+ET model should shift the prediction from the mean, and the recency offset correction. The only learned parameters are $\beta_0$, $\beta_1$ and $\beta_2$.
\end{enumerate}

Both variants use the same 150 latest startest activities from training.
...
\item Third, a hybrid approach was developed in which a ridge regression is fitted to predict the residuals of the RF and ET ensemble, using the RF and ET prediction and the LLM forecast as inputs, so that systematic recency drift and LLM-derived signal can both be incorporated into a corrected final prediction.
...
Figure~\ref{fig:scatterLLMcorrected} summarises performance across all LLM forecasting configurations on two validation sets (Set~A, $n=300$; Set~B, $n=171$). The best narrative similarity grade is 88.5 (a ``B+'', corresponding to forecasts that are on average between ``mostly accurate'' and ``highly accurate''), achieved by the RF-forced method with full context (RAG+KNN+S1+S2). The minimal-prompt baseline achieves the highest $R^2$ for rating prediction but the worst narrative similarity, while RF-forced methods dominate on both narrative grade and pairwise ordering probability. These patterns are consistent across both validation sets.
...
Overall, the forecasting system developed is capable of forecasting evaluation ratings significantly above chance on out-of-time activities. Compared to prior work \citep{ashtonPuzzleMissingPieces2023}, a value consistent with their adjusted $R^2$ on the training set is reported. An $R^2$ of 0.34 and an adjusted $R^2$ of 0.29 are reported for within-training set correlations on primarily world bank ratings, while others report at maximum an adjusted $R^2$ of ~0.3 \citep{goldembergMindingGapAid2025}. No comparable out-of-time, time-ordered split analysis was identified in the literature. As expected for forecasting under data distribution shift, results on the out-of-time validation set were somewhat weaker, with an $R^2$ on the validation set of \Rsqnollmfrac without LLM features for the RF+ET model, and an $R^2$ on the validation set of \Rsqfrac when incorporating the language model forecasting results correction + recency model. In order to compare the model with results from prior literature, adjusted $R^2$ on the training set of the best statistical model was calculated. It was found that the model has an adjusted $R^2$ of \Rsqadjfrac, similar to the $R^2$ on the validation set of \Rsqfrac, indicating overfitting is unlikely. This is also similar to the adjusted $R^2$ in the published literature on overall success ratings.

Several factors were found to improve model performance. The recency correction was found to provide better results than averaging LLM and statistical models, due to their widely differing forecasting skill. As the ET model performed similarly to the RF model with a slightly different algorithm, it was found that averaging the random forest model with the extra trees model also improved forecasting performance. Further successful techniques are listed below.
...
\midrule
RF+ET + LLM Forecast + recency & \textbf{0.255} & \textbf{0.817} & 0.593 & \textbf{0.757} & \textbf{0.547} & 0.724 \\[3pt]
RF+ET + recency & 0.240 & 0.825 & 0.603 & 0.743 & 0.537 & 0.724 \\[3pt]
RF+ET + LLM Forecast + recency (rounded) & 0.219 & 0.836 & \textbf{0.525} & \textbf{0.757} & \textbf{0.547} & \textbf{0.758} \\[3pt]
RF+ET (default params) & 0.215 & 0.839 & 0.628 & 0.723 & 0.530 & 0.715 \\[3pt]
XGBoost, no LLM features & 0.196 & 0.848 & 0.645 & 0.693 & 0.513 & 0.701 \\[3pt]
RF+ET only & 0.192 & 0.851 & 0.645 & 0.693 & 0.507 & 0.724 \\[3pt]
Ridge GLM + Random Forest (mean) & 0.185 & 0.854 & 0.649 & 0.690 & 0.523 & 0.707 \\[3pt]
XGBoost only & 0.184 & 0.855 & 0.638 & 0.693 & 0.523 & 0.702 \\[3pt]
RF+ET, no LLM features & 0.171 & 0.862 & 0.657 & 0.713 & 0.520 & 0.709 \\[3pt]
Ridge GLM & 0.142 & 0.877 & 0.671 & 0.650 & 0.480 & 0.678 \\[3pt]
Mode of reporting-org score baseline & 0.102 & 0.897 & 0.579 & 0.700 & 0.530 & 0.712 \\[3pt]
Ridge Baseline (risks + org only) & 0.017 & 0.938 & 0.709 & 0.570 & 0.433 & 0.669 \\[3pt]
\bottomrule
\end{tabular}
...
\begin{figure}[!htbp]
  ...
  \caption{The scatter of observed vs predicted points on the validation set for the LLM-corrected RF prediction. $R^2=\Rsqfrac$, Pairwise probability of \PairwiseModel\,\%.   }
  \label{fig:scatterllmpredcorrected}
\end{figure}
\FloatBarrier
...
The results from the model to predict cost-effectiveness are shown in Table \ref{tab:outcome_model_performance}. When dividing by each activity's disbursement for those that are marked as dollar-per-unit in Table \ref{tab:outcome_model_performance}, and looking at all cost-effectiveness outcomes except ratings, the correlation on Z-scores drops to 0.05, (95\% CI: -0.03, 0.13) and pairwise probability of 60\% (95\% CI: 54\%, 67\%) ($N_{\mathrm{val}}=299$), compared to an $R^2$ of 0.10 (95\% CI -0.0982, 0.2625) and a pairwise probability of 68\% (95\% CI: 0.62, 0.74) ($n_val$=84) when forecasting the Z-score for CO$_2$ emission reductions, generation capacity, and water connections. While pairwise ordering ability for cost-effectiveness outcomes is statistically significant, the overall ability is weak and appears to be largely a function of expenditure and sector clusters.

Interpreting these results, although it is likely that cost-effectiveness bests a simple baseline of predicting the mean of the aggregate cost-effectiveness Z-score for the validation set with 95\% confidence (see Figure \ref{tab:outcome_model_performance}), this absolute prediction accuracy cannot be distinguished from the baseline with 95\% confidence. Despite this weak performance in absolute accuracy, ranking cost-effectiveness performs modestly better. The model is able to distinguish cost-effectiveness between pairs of activities, at a rate of 60\% accuracy. The model also beats the ranking performance of a random ordering (50\%) with 95\% confidence.

The top two features of the model are UMAP x axis (higher x-axis values reduce cost-effectiveness), and the planned duration (longer durations increase cost-effectiveness). This stands in contrast with findings in the literature that better ratings are correlated with shorter durations. Although the model is not sufficiently strong to lend much confidence in this finding, the model interactions suggest that longer planned activities are more cost-effective.

\begin{table}
\caption{Predictive performance of cost-effectiveness outcome RF model on the validation set. R$^2$, Pairwise ordering probability, and Spearman correlation are shown with 95\% bootstrap confidence intervals. In general, only the aggregate had sufficient data to produce a meaningfully predictive model, although the model is only able to distinguish more cost-effective activities 60\% of the time. Cost-effectiveness outcomes are sorted by R$^2$ (descending). The Benefit-Cost Ratios had identical forecasts for all examples, hence a pairwise and Spearman could not be properly calculated. All other cost-effectiveness outcomes had fewer than 10 items in training or validation. 95\% confidence intervals are determined via bootstrap over the prediction set, which was only computed once.}
...
Water Connections  Connections ($log_{10}(connections)$) & -0.01 [-0.39, 0.13] & 66 [54, 77] & 0.43 [0.07, 0.69] & 0.70 & 71 / 25 / 25 \\
Economic Rate Of Return  Percent (percent) & -0.01 [-0.19, 0.03] & 53 [45, 60] & 0.09 [-0.14, 0.28] & 15.39 & 293 / 103 / 103 \\
CO$_2$ Emission Reductions  Tonnes CO2E ($log_{10}(tonnes\ CO_2e)$) & -0.03 [-0.32, 0.00] & 41 [27, 58] & -0.28 [-0.62, 0.18] & 1.46 & 38 / 23 / 23 \\
...
\FloatBarrier
\section{Conclusion}
The findings regarding each research question posed in the introduction are summarized below.

\textbf{How do judgemental forecasting methods compare to statistical models in forecasting international aid overall success ratings and quantitative outcomes?}
...
\textbf{What aspects of the activity available in the dataset at the beginning of the activity lead to higher or lower ratings?}
