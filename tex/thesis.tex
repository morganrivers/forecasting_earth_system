...
\begin{itemize}[noitemsep,topsep=0pt]
 ...
 \item the mean of 5 World Governance Indicators score \citep{kaufmannWorldwideGovernanceIndicators2024}. The mean was over control of corruption, government effectiveness, political stability, regulatory quality, and rule of law. \citep{goldembergMindingGapAid2025} \citep{ndikumanaImpactForeignAid2017}
...
The model for the outcome tag was judged as ineligible for test-set prediction unless the Brier skill score was strictly positive \emph{and} the accuracy skill exceeded 0.5 percentage points above the majority-class baseline to prefer the baseline in the case of a tie. Otherwise the chosen model was reverted to the constant baseline, indicating low confidence that the RF+ET ensemble would perform better than the validation set. 
...
An ablation experiment for all tags was conducted on the validation set. The RF+ET model uses approximately 62 independent features (47 explicit base features plus 15 sector cluster columns), which may not be appropriate for all outcome tags. A series of experiments were conducted to assess which features should be used per-tag.
...
\emph{deepseek-V3.2} (with reasoning enabled) was selected %Llama 70B \citep{touvronLlama2Open2023}%
as the forecasting model for overall ratings, due to its high performance on general reasoning ability benchmarks and low cost. \emph{gemini-3-pro-preview} was also used, due to its superior performance on reasoning benchmarks. Lastly \emph{gemini-2.5-flash
...
Each example activity in the few-shot block included (i) the activity title, (ii) a short ``risks'' summary, (iii) the retrospective mock forecast analysis, and (iv) the final evaluation outcome label. 
...
An ablation experiment to drop each of the groups individually revealed that governance indicators (World Governance Indicators and CPIA score) and missingness-flag features (indicators of which LLM-extracted fields were absent) flags were introducing more noise than performance gains. These combined 17 features (6 governance, 11 uncertainty) were thus dropped from all tags, leaving 45 features available for further feature selection. The drop was confirmed to improve scores on the K-fold temporal validation on the training set.
