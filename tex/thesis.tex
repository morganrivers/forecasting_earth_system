...
While cost-effectiveness provides a useful alternative to ratings that are less likely to be gamed, accurate extraction of quantitative outcomes per activity and total expenditure are challenging. Challenges include lack of consistent reporting and definitions of quantitative units, differing choices of which proxy for cost-effectiveness are used, and differing definitions of total expenditure (such as whether expenditure is counted from only the reporting organization, or additional funding from recipient governments).
...
The dataset and extraction pipeline used in this thesis are available at \url{https://github.com/morganrivers/iati_extraction}. The repository contains ratings, binary outcome tags, quantitative outcome measurements, LLM-extracted input features, forecast texts, and grading results for approximately 1,800 environmental and sustainability projects from the World Bank, BMZ, UK FCDO, and the Asian Development Bank, organized into train, validation, and test splits. An interactive web tool for exploring the data and applying the forecasting system is available at \url{https://forecast-iati.onrender.com}. The code used to generate the thesis results is available on request.
