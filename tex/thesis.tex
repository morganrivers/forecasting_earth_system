...
The table below summarises the key experimental methods tested in this thesis.
\textcolor{green!60!black}{$\checkmark$}~indicates improvement on both the validation and held-out test set;
\textcolor{red}{$\times$}~indicates the method did not generalise to the held-out test set (or failed on validation);
{\color{gray}?}~indicates ambiguous, mixed, or no clear improvement.
LLM forecast methods were assessed on the validation set only.

\textbf{Statistical model (overall rating).}
The baseline methods were a mode-of-organisation predictor and a ridge regression using only an LLM-generated risk grade and organisation identity.
...
A linear ridge regression correction fitted on start year was applied to the ensemble output.
...
XGBoost was also assessed on the held-out set and underperformed RF+ET.

\textbf{LLM forecast (overall rating and narrative quality).}
The baseline was a minimal single-stage prompt with activity metadata and a generated summary.
...
\midrule
\multirow{6}{3.1cm}{\textbf{Statistical Model (Overall Rating)}}
  ...
  & Linear ridge regression adjustment to RF based on start year & {\color{gray}?} \\
  & Simple average with LLM forecast                    & {\color{gray}?} \\
  & LLM ridge adjustment of forecast                    & \textcolor{red}{$\times$} \\
  & XGBoost                                             & \textcolor{red}{$\times$} \\
  & Simple average with LLM forecast                    & {\color{gray}---} \\
\midrule
\multirow{8}{3.1cm}{\textbf{LLM Forecast (Overall Rating)}}
  & \multicolumn{2}{l}{\textit{Narrative quality (validation set):}} \\
  & \quad LLM instructed to forecast statistical model's rating & \textcolor{green!60!black}{$\checkmark$} \\
  ...
  & \quad S1/S2 additional reasoning                    & \textcolor{red}{$\times$} \\
  & \quad KNN few-shot retrieval                        & {\color{gray}---} \\
  & \quad Statistical model $\to$ LLM prompt            & \textcolor{green!60!black}{$\checkmark$} \\
\midrule
\textbf{Cost-Effectiveness}
  & No methods attempted for held-out test set: validation performance not above chance & \\
\midrule
\textbf{Outcome Tags}
